...
with st.sidebar:
    ...
    st.info("🔒 حماية البيانات: **لا يتم تخزين أي كود إطلاقاً** (Zero-Persistence).")
    st.markdown("---")
...
rules = [
    {
        ...
        "scenario": "تمكين المهاجم من الوصول الكامل لتسريب بيانات السحابة أو تدمير الموارد.",
    ...
    },
    {
        ...
        "severity": "🟠 عالية (High)",
        "scenario": "سهولة اختراق الحسابات وقواعد البيانات بمجرد قراءة الكود المصدري.",
    ...
    },
    {
        ...
        "scenario": "استغلال الصلاحيات لإجراء طلبات غير مصرح بها باسم التطبيق.",
        "fix": "تشفير المفاتيح واستدعاؤها عبر متغيرات البيئة."
    }
]

safe_pattern = re.compile(
    r"(os\.getenv|os\.environ|process\.env|env\.get|get_secret|vault)\.",
    re.IGNORECASE
)
...
# خيارات المدخلات
tab1, tab2 = st.tabs(["مباشرة لصق الكود", "📁 رفع ملف برمجي"])

code_content = ""

with tab1:
    user_code = st.text_area(
        "أدخل الكود البرمجي هنا للفحص:",
        height=220,
...
with tab2:
    uploaded_file = st.file_uploader("اختر ملف برمجي (.py, .js, .txt)", type=["py", "txt", "js"])
    if uploaded_file is not None:
        try:
            code_content = uploaded_file.getvalue().decode("utf-8")
            st.caption(f"الملف المحدد: `{uploaded_file.name}`")
        except UnicodeDecodeError:
            st.error("تعذر قراءة الملف. ارفع ملفاً نصياً بترميز UTF-8.")
